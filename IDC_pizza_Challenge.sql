...
SELECT DISTINCT(category) AS unique_pizza_category
FROM pizza_types;
...
SELECT od.order_id, p.pizza_id, SUM(od.quantity * p.price) AS total_order_value
FROM order_details od
JOIN pizzas P ON od.pizza_id = p.pizza_id
GROUP BY od.order_id, p.pizza_id;

-- 4. Total quantity sold per pizza category (JOIN, GROUP BY).
SELECT pt.category, SUM(od.quantity) AS total_qantity_sold
FROM order_details od
JOIN pizzas p ON od.pizza_id = p.pizza_id
...
SELECT p1.pizza_type_id, ROUND((MAX(p2.price) - MIN(p1.price)),2) AS price_difference
FROM pizzas p1
